...
import com.sky.app.library.component.footerView.BaseFooterView;
import com.sky.app.library.component.footerView.FooterViewListener;
import com.sky.app.library.component.footerView.SimpleFooterView;
...
public class SwipeRecyclerView extends FrameLayout
    ...
    private BaseFooterView mFootView;               //底部加载框
    private RecyclerView recyclerView;              //列表组件
    private SwipeRefreshLayout mRefreshLayout;      //下拉刷新组件

    private LayoutManager mLayoutManager;
    private OnLoadListener mListener;               //刷新接口
    private SpanSizeLookup mSpanSizeLookup;         //grid格式
    private DataObserver mDataObserver;             //适配器绑定
    private WrapperAdapter mWrapperAdapter;         //自定义适配器

    private boolean isEmptyViewShowing;             //是否展示空视图 true：展示 false：不展示
    private boolean isLoadingMore;                  //是否可以加载更多 true：可以 false：不可以
    private boolean isLoadMoreEnable;               //是否支持加载更多 true：支持 false：不支持
    private boolean isRefreshEnable;                //是否支持刷新 true：支持 false：不支持

    private int lastVisiablePosition = 0;           //可见的最后一条数据编号
    ...
    private void setupSwipeRecyclerView() {
        ...
        View view = LayoutInflater.from(getContext()).inflate(R.layout.app_swipe_recyclerview, this);
        mRefreshLayout = (SwipeRefreshLayout) view.findViewById(R.id.SwipeRefreshLayout);
        mRefreshLayout.setColorSchemeResources(R.color.main_colorPrimary);
        ...
        recyclerView.addOnScrollListener(new OnScrollListener() {
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if(!isLoadMoreEnable || isRefreshing() || isLoadingMore) return;

                lastVisiablePosition = getLastVisiblePosition(recyclerView);
