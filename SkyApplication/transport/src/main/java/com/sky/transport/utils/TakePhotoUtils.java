...
public class TakePhotoUtils {
    ...
    public void takePoto(String imageName){
        ...
        if (intent.resolveActivity(activity.getPackageManager()) != null) {
            File tempFile = new File(Constants.Path.TRANSPORT_IMAGE_CACHE_PATH);
    ...
    public String onActivityResult(int requestCode, int resultCode, Intent data,
        ...
        switch (requestCode){
            ...
            case PHOTO_REQUEST_CAREMA://拍照
                File file = new File(Constants.Path.TRANSPORT_IMAGE_CACHE_PATH + imageName);
                Uri uri = FileProvider.getUriForFile(activity, "com.sky.transport.fileProvider", file);
                L.msg("3->" + uri.getPath());
                if (isCrop){//裁剪
                    crop(uri);
                }else{
    ...
    private void crop(Uri uri) {
        ...
        cropUrl = Uri.parse("file://" + Constants.Path.TRANSPORT_IMAGE_CACHE_PATH + imageName);
