...
public class MyOrderAdaptor extends BaseRecyclerAdapter<OrderDetail> {
    ...
    public int getItemLayoutId(int viewType) {
        if (0 == mPage){
            return R.layout.transport_order_going;
        }else{
            return R.layout.transport_order_finished_or_cancel;
        }
    }
    ...
    public void bindData(RecyclerViewHolder holder, final int position, OrderDetail item) {
        ...
        if (0 == mPage){
            ...
            holder.setClickListener(R.id.driver_cancel_btn, new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if (null != onClickListener){
                        onClickListener.onClick(v, position);
